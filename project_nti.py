...
model.save("saved_modelsignature_2.keras")

"""# Test"""

from keras.models import load_model
model = load_model("saved_modelsignature_2.keras")
...
def verify_random_signature(model, folder_path):
    try:
        ...
        if not all_images:
            print(" No signature images found in the folder.")
            return
        ...
        img = preprocess_signature(full_path)
        score = model.predict(img)[0][0]
        result = "GENUINE" if score > 0.5 else "FORGED"


        print(f"\n Selected: {selected_image}")
        print(f" Prediction Score: {score:.4f} → {result}")

        original_img = cv2.imread(full_path, cv2.IMREAD_GRAYSCALE)
        color = 'green' if result == "GENUINE" else 'red'
        plt.imshow(original_img, cmap='gray')
        plt.title(f"Prediction: {result}", color=color)
        plt.axis('off')
        plt.show()
...
dataset_path = "/content/drive/MyDrive/signature/full_org"
verify_random_signature(model, dataset_path)

dataset_path = "/content/drive/MyDrive/signature/full_forg"
verify_random_signature(model,dataset_path)